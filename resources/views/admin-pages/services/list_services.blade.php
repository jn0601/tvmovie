        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <a href="{{URL::to('/admin/services/create')}}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Thêm mới</a>
            </ul>
                ...
                <tr>
                  ...
                  <td class="text-center"><span class="text-ellipsis">
                      <?php if ($item->status == 1) { ?>
                        <a href="{{URL::to('/admin/unactivate-services-status/'.$item->id)}}"><span class="fa fa-toggle-on status_button"></span></a>
                        <?php } else { ?>
                        <a href="{{URL::to('/admin/activate-services-status/'.$item->id)}}"><span class="fa fa-toggle-off status_button"></span></a>
                        <?php } ?>
                    </span>
                  </td>
                  <td class="text-right">
                    <a title="Sửa" href="{{URL::to('admin/services/'.$item->id .'/edit')}}"
                      class="btn btn-info btn-xs btn-sm"><i class="fa fa-pencil"></i> </a>
                    
                    <form class="form_delete" method="post" action="{{route('admin/services.destroy',$item->id)}}" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')">
                          @method('delete')
                          @csrf
