        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <a href="{{URL::to('/admin/banners/create')}}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Thêm mới</a>
            </ul>
            ...
            @endphp
            {{-- {!! Form::open(['url'=>'admin/banners-search', 'method'=>'get', 'enctype'=>'multipart/form-data']) !!}
            ...
            <table class="table table-striped projects bulk_action">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <img src="{{URL::to('public/backend/uploads/banners/'.$banners->image)}}" class="list-image">
                  </td>
                  ...
                  <td class="text-center">
                    @if($banners->category_id != 0)
                        @foreach($dataCategory as $key => $cate)
                          @if($cate->id == $banners->category_id)
                            <span>{{$cate->name}}</span>
                          @endif
                        @endforeach
                    @endif
                  ...
                  <td class="text-center"><span class="text-ellipsis">
                      {{-- <a>{{$banners->status == 1 ? 'Hoạt động' : 'Không hoạt động'}}</a> --}}
                      <?php if ($banners->status == 1) { ?>
                        <a href="{{URL::to('/admin/unactivate-banners-status/'.$banners->id)}}"><span class="fa fa-toggle-on status_button"></span></a>
                        <?php } else { ?>
                        <a href="{{URL::to('/admin/activate-banners-status/'.$banners->id)}}"><span class="fa fa-toggle-off status_button"></span></a>
                        <?php } ?>
                    </span>
                  </td>
                  <td class="text-right">
                    <a title="Sửa" href="{{URL::to('admin/banners/'.$banners->id .'/edit')}}"
                      class="btn btn-info btn-xs btn-sm"><i class="fa fa-pencil"></i> </a>
                    
                    <form class="form_delete" method="post" action="{{route('admin/banners.destroy',$banners->id)}}" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')">
                          @method('delete')
                          @csrf
