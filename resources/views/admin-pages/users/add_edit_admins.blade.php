				...
				<div class="x_panel">
					<div class="x_title">
						...
						<ul class="nav navbar-right panel_toolbox">
							<a href="{{URL::to('/admin/admins/list-admin')}}" class="btn btn-primary btn-sm"> <i class="fa fa-sign-out"></i> Quay lại </a>
						</ul>
					...
					<div class="x_content LVR_list-admin">
						@if ($data == '')
							{!! Form::open(['url'=>'admin/admins/store', 'method'=>'POST', 'enctype'=>'multipart/form-data']) !!}
							{{ csrf_field() }}
						@else
							{!! Form::open(['route'=>['admin/admins.update',$data->id], 'method'=>'PUT', 'enctype'=>'multipart/form-data']) !!}
							{{-- @method('PUT') --}}
							{{ csrf_field() }}
						@endif
                          ...
                          $type_value = isset($data['type']) ? $data['type'] : "";
                        ...
                        @if($get_type == 99)
                          @if ($data != '')
                            ...
                            <div class="field item form-group">
                              <label class="col-form-label col-md-2 col-sm-2">Loại tài khoản<span class="required">*</span></label>
                              <div class="col-md-10 col-sm-10 input-group input-group-sm">
                                <select name="type" class="form-control">
                                  <option {{$data->type == 1 ? 'selected' : ''}} value="1">Quản trị viên</option>
                                  <option {{$data->type == 99 ? 'selected' : ''}} value="99">Lập trình viên</option>
                                </select>
                              </div>
                            </div>
                            @endif
                          ...
                          <div class="field item form-group">
                            ...
                            <div class="col-md-10 col-sm-10 input-group input-group-sm">
                              <select name="type" class="form-control">
                                ...
                                <option selected value="99">Lập trình viên</option>
                              </select>
                            </div>
                          </div>
                          @endif
                        ...
                        @if ($data != '')
                          @if ($data->username != "root")
                          <div id="div-role" class="field item form-group">
                            <label class="col-form-label col-md-2 col-sm-2">Vai trò</label>
                            <div class="col-md-10 col-sm-10 input-group input-group-sm">
                              <select name="role_id[]" class="custom-select js-example-templating" multiple="multiple">
                                ...
                                @foreach($listRole as $key => $value)
                                  <option 
                                    @php
                                      foreach ($subRole as $key => $sub_role) {
                                        if ($sub_role->role_id == $value->id) {
                                          echo 'selected';
                                        } else {
                                          echo '';
                                        }
